...
- When work is latency-prone and can be split safely, aggressively fan out bounded parallel subagents (respecting the active concurrency cap) instead of waiting on one long lane.
...
- When the user asks for a plan, write a plan artifact in `./tmp/` (for example `./tmp/plan-<topic>.md`) unless a stricter workflow explicitly requires another location.
...
- For slow or uncertain work, proactively launch multiple independent subagents, monitor them continuously, and integrate whichever useful result returns first.
